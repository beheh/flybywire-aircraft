...
impl A320CabinIntercommunicationDataDirector {
    ...
    fn new(context: &mut InitContext, number: usize) -> Self {
        Self {
            ...
            no_ped_auto_setting: AutoLightsSetting::Gear,
        }
    }
    ...
    fn determine_outputs(
        ...
        let no_smoking_signs = self.determine_no_smoking_signs(inputs);
        ...
        let no_smoking_signs_arinc = no_smoking_signs && !inputs.no_ped_installed; // Non-smoker aircraft
        ...
        discrete_word_1.set_bit(23, true); // Cabin Ready installed
    ...
    fn determine_no_smoking_signs(
        ...
        // Assume a non-smoker aircraft for now - this means the no smoking switch only controls the
        // exit signs, and the no smoking signs are always on.
        !inputs.no_ped_installed
    }

    fn determine_exit_signs(&self, inputs: &A320CabinIntercommunicationDataDirectorInputs) -> bool {
        // These are usually controlled together with the No Smoking switch, however for a
        // non-smoker aircraft only these are controlled by the No Smoking switch.
        ...
        if inputs.cpc_1_excessive_alt || inputs.cpc_2_excessive_alt || exit_command {
            true
        } else if exit_auto {
...
mod tests {
    ...
    #[case(Some(1))]
    #[case(Some(2))]
    fn auto_no_ped_with_gear_up_are_off(#[case] failed_cids_director: Option<usize>) {
        let mut test_bed = test_bed_with()
            .optionally_failed_cids_director(failed_cids_director)
            .engines_running()
            .flaps_out(true)
        ...
        test_bed.assert_no_portable_devices_signs(false);
    }
